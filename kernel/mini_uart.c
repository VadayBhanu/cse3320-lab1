...
void uart_irq(void) {
    ...
    if (IS_RECEIVE_INTERRUPT(iir)) {
        ...
        while (1) {
            ...
            // Quest 12: read received character
            c = uart_try_recv();

            // no more characters in RX FIFO
            if (c == -1)
                break;

            V("char %d", c);

            // Quest 12: use UART input to control virtual timers
            test_ktimer2(c);
        }
    }
}
...
void uart_init(void) {
    ...
    // flush FIFO
    put32(AUX_MU_IIR_REG, FLUSH_UART);


    // Enable mini uart
    put32(AUX_ENABLES, 1);

    // Disable receiver/transmitter temporarily
    put32(AUX_MU_CNTL_REG, 0);


    // Disable receive and transmit interrupts initially
    put32(
        AUX_MU_IER_REG,
        (3 << 2) | (0xf << 4)
    );
    ...
    // Quest 12: enable UART RX interrupt
    {
